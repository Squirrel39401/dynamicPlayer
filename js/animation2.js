(function (dynamic_play) {
    Array.prototype.remove = function (item) {
        var index = this.indexOf(item);
        if (index >= 0) return this.splice(index, 1);
        return item;
    }
    function throttle(func, timeout, context) {
        var args;
        var timer;
        var previous;
        return function () {
    ...
    class APNode {
        ...
        findAnimation(name) {
            return this.skeleton.data.animations.filter(action => action.name == name)[0]
        }
    ...
    class AnimationPlayer {
        ...
        changeSkin(path, sprite, skelType = 'skel', onload = () => {
            this.playSpine(sprite)
        }, onerror = () => { }) {
            this.originalNodes.push(this.nodes)
            this.nodes = []
            this.loadSpine(path, skelType, onload, onerror)
        }
    ...
    class AnimationPlayerPool {
        constructor(size, pathPrefix, thisName) {
            if (!self.spine) return console.error('spine 未定义.');

            this.name = thisName;
            this.animations = new Array(size ? size : 1);
            for (var i = 0; i < this.animations.length; i++) this.animations[i] = new dynamic_play.AnimationPlayer(pathPrefix);
    ...
    let BUILT_ID = 0;
    let DynamicWorkers = new Array(2);
    class DynamicPlayer {
        constructor(pathPrefix) {
            ...
            if (offscreen) {
                ...
                for (var i = 0; i < workers.length; i++) {
                    ...
                    this.canvas.className = 'animation-player-dpbf';
        ...
        play(sprite) {
            ...
            if (sprite.background && !sprite.dynamicBackground) {
                ...
                let dynamic = new dynamic_play.DynamicPlayer('./')
    ...
    Object.assign(dynamic_play, {
        throttle, observeSize, lerp, ease, CubicBezierEase, TimeStep, APNode, AnimationPlayer, AnimationPlayerPool, BUILT_ID, DynamicWorkers, DynamicPlayer
    })
})(window.dynamic_play = window.dynamic_play || {})